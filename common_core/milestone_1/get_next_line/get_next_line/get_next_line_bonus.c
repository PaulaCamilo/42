...
/*   Updated: 2024/11/26 13:55:44 by ppaula-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static t_list	*stash[MAX_FD]; // reserve of characters read with the read() function;
	...
	if (stash[fd] == NULL) // this works the case if the the file is empty
		return (NULL);
	// 2. extract from stash to line
	extract_line(stash[fd], &line); // since we are not going to modify the stash, we don't need to address, opposite to the line which we will change it so we need to change its memory address
	// 3. clean up stash
	clean_stash(&stash[fd]); // here we need to address our stash because we are going to clean all the content generated until here, modifying its memory
